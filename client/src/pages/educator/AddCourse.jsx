    ...
    chapters.map((chapter) => {
      if (chapter.chapterId === currentChapterId) {
        const newLecture = {
          ...
          chapter.chapterContent.slice(-1)[0].lectureDuration + 1 : 1,
          lectureId: uniqid()
        };
...
const handleSubmit = async (e) => {
  ...
  return (
    <div className='h-screen overflow-scrool flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>

      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500' action="">
        ...
        <div className='flex items-center justify-between flex-wrap'>
          <div className='flex flex-col gap-1'>
            <p>Course Price</p>
            <input onChange={e => setCoursePrice(e.target.value)} value={coursePrice} type='number' placeholder='0' className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' required></input>
          </div>
            ...
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
              ...
              {!chapter.collapsed && (
                <div className='p-4'>
                  ...
                  onClick={()=> handleLecture('add', chapter.chapterId)}>+ Add Lectures</div>
                </div>
              )}
